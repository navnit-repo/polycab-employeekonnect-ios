...
@implementation PMCalendarView
{
    ...
    CGRect rectBetweenLeftRightArrows;
    NSInteger fontSize;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    rectBetweenLeftRightArrows = CGRectMake(leftArrowRect.origin.x + leftArrowRect.size.width,
                                            leftArrowRect.origin.y,
                                            rightArrowRect.origin.x - leftArrowRect.origin.x - leftArrowRect.size.width, leftArrowRect.size.height);
}
...
- (void) tapHandling: (UIGestureRecognizer *)recognizer
{
    ...
    else if(CGRectContainsPoint(rectBetweenLeftRightArrows, point)) {
        // user must have tapped on the month year string
        
        NSLog(@"I am tapped on the month and year string");
        
        if ([_delegate respondsToSelector:@selector(tappedOnMonthYear:)])
        {
            [_delegate tappedOnMonthYear:self.currentDate];
        }
    }
