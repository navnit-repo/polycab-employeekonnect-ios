...
//    self.initialSize = size;
    ...
    CGRect calendarRectWithArrowInsets = CGRectMake(0, 35
...
- (void) tappedOnMonthYear: (NSDate *)currentDate {
    NSLog(@"- (void) tappedOnMonthYear: (NSDate *)currentDate;");
    DVMonthYearPicker* monthYearPicker = [[DVMonthYearPicker alloc]  initWithFrame:CGRectMake(0, 200, 300, 200)];
    monthYearPicker.monthYearDelegate = self;
...
-(void) selectedMonth:(int) monVal year:(int) yearVal
{
    ...
    NSLog(@"selected month year from picker: month %d, year %d = ", monVal, yearVal);
    ...
    [components setMonth: monVal];
	[components setDay:1];

    [self.digitsView setCurrentDate:[calendar dateFromComponents:components]];
}
