...
@implementation BarCodeScanField
...
-(void) configureViewCellFor:(DotFormElement*) formElement
{
    ...
    UIImage *iconImage = [UIImage imageNamed:@"icon_barcode_scan"];
    
    self.scanButton = [MXButton buttonWithType:UIButtonTypeCustom];
    self.scanButton.frame = CGRectMake( 0.0f, 4.0f, self.frame.size.height-8, self.frame.size.height-4);
    self.scanButton.backgroundColor = [UIColor clearColor];
    [self.scanButton setImage:iconImage forState:UIControlStateNormal];
    self.scanButton.elementId = dotFormElement.elementId;
    
    [self.editText setRightView:self.scanButton];
